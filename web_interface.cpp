...
void UTflix::view_movie_details(int id,int user_id) {
	int flag = 0;
  std::string body = "";
  body += set_movie_details_body();
		if (check_if_movie_existed(id)) {
			...
			if (flag)
				print_recommendation_films(id,user_id);
			return;
		}
	//	throw NotFound();
}
...
std::string Movie::view_details() {
  std::string body = "";
  body += "<tr> <td> <a id=\"film\" href=\"/film_details\" name=\"film_id\"";
  body += "type=\"hidden\" value=" + std::to_string(film_id)+ ">";
  ...
  body += "<td><button class=\"button\" type=\"submit\">Buy</button></td>";
  body+= "</tr></table>"  ;
  return body;
}
...
std::string UTflix::set_publisher_home_body(){
  ...
  body += "<div class=\"filter_div\"><form action=\"/money\" method=\"post\">";
...
std::string Movie::view_published_details() {
  ...
  body += "<td>" + movie_information["year"] + "</td>";
  body += "<td>" + movie_information["director"] + "</td>";
  body += "<td><a id=\"film\" href=\"/delete\"></a>";
  body += "<button class=\"button\" type=\"submit\">Delete</button></td>";
  body+= "</tr>";
  return body;
}
...
std::string UTflix::set_profile_body(){
  ...
  body += "hover#film, a:active#film, a:visited#film {";
  body += "color:red;text-decoration: underline;}";
  body += "input[type=text] {width: 100%;box-sizing: border-box;";
  ...
  body += "color: white;}.button {background-color: rgb(61, 96, 56);";
  body += "border: none;color: white;padding: 15px 32px;";
  body += "text-align: center;text-decoration: none;display: inline-block;";
  body += "font-size: 16px;margin: 4px 2px;cursor: pointer;}";
  body += ".button:hover {background-color: red;}";
  ...
  body += "<div class=\"money_div\"><form action=\"/money\" method=\"post\">";
  body += "<input type=\"text\" name=\"money\" placeholder=\"Increase money\">";
  body += "<button class=\"button\" type=\"submit\">Charge</button> <br>";
  body += "</form></div><div class=\"filter_div\"><form action=\"/money\" method=\"post\">";
  body += "<input type=\"text\" name=\"director\" placeholder=\"Filter by director\">";
  body += "<button class=\"button\" type=\"submit\">Filter</button> <br></form>";
  body += "</div><div class=\"logout_div\"><a href=\"/loggedout\">Logout</a></div>";
...
std::string Movie::view_bought_details() {
  ...
  body += "<td>" + movie_information["year"] + "</td>";
  body += "<td>" + movie_information["director"] + "</td>";
  body += "<td><a id=\"film\" href=\"/rate\"></a>";
  body += "<button class=\"button\" type=\"submit\">Rate</button></td>";
  body+= "</tr>";
  return body;
}
